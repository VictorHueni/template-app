...
### Phase 0: Final Steps (Commit Prerequisites & Configuration)
...
1. Run `git log --oneline -1` to confirm commit message is correct.
2. Run `./mvnw clean compile -pl backend` to ensure post-commit build is clean.
3. Verify no broken references: `./mvnw verify -pl backend -DskipTests`.
4. Document in ticket/PR: "Phase 0 prerequisites validated and committed."
...
**Test:**
1. Run a test with `@SpringBootTest` with the `integration` profile active.
2. Inject `JwtDecoder` in the test to verify the bean is registered: `@Autowired JwtDecoder jwtDecoder`.
3. Decode a token generated by `TestJwtUtils.getUserToken()` using the injected bean.
4. Verify no exceptions are thrown and the decoded token contains the expected claims.
5. Expected outcome: The JwtDecoder bean is available and successfully validates JWTs signed with the test secret.
...
**Test:**
1. Run a secured integration test that extends `AbstractSecuredRestAssuredIT` (e.g., `GreetingControllerSecuredIT`).
2. Verify the test compiles without errors referencing removed Keycloak classes.
3. Execute a protected endpoint with `RestAssured.given().header("Authorization", "Bearer " + getUserToken())`.
4. Verify the request succeeds (200 status) and the authenticated user is correctly propagated.
...
3. Run the complete test suite: `./mvnw verify -Dspring.profiles.active=integration`.
...
### Phase 1: Final Steps (Commit Authentication Layer)
...
git commit -m "refactor(test): Phase 1 - Mock authentication layer (Virtual Gateway)

**Changes:**
- Create local JWT token generation utilities
  * Support configurable claims (username, roles, issuer)
  * Sign tokens with test secret for local validation
  * Support user and admin token types

- Configure test profile JWT validation
  * Define JwtDecoder bean with test secret
  * Restrict to 'integration' profile only
  * No production code changes

- Update secured test base classes
  * Remove testcontainer OAuth2 dependencies
  * Use local token generation instead
  * Propagate authenticated context to tests

**Why:**
- Eliminates Keycloak container startup overhead (~15-20s saved per test run)
- Enables deterministic JWT testing without external service
- Mocks the Gateway's role in the architecture

**Testing:**
- All secured endpoints authenticate with mock tokens
- Role enforcement verified (403 for insufficient permissions)
- Invalid/missing token handling verified (401)
- Identity claims correctly propagated (createdBy, etc.)
"
```

**Verification After Commit:**
1. Run `git log --oneline -1` to confirm commit message.
2. Test full authentication layer: `./mvnw test -Dtest='*Auth*,*Secured*' -pl backend`.
3. Verify build quality: `./mvnw verify -pl backend` (includes code analysis).
4. Document in ticket/PR: "Phase 1 authentication layer complete. Mock JWT generation active."
